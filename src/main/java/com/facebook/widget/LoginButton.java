...
public class LoginButton extends Button {
    ...
    public LoginButton(Context context, AttributeSet attrs) {
        ...
        if (attrs.getStyleAttribute() == 0) {
            ...
            this.setTextColor(getResources().getColor(0));
            this.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                    getResources().getDimension(0));
            this.setPadding(getResources().getDimensionPixelSize(0),
                    getResources().getDimensionPixelSize(0),
                    getResources().getDimensionPixelSize(0),
                    getResources().getDimensionPixelSize(0));
            this.setWidth(getResources().getDimensionPixelSize(0));
            this.setHeight(getResources().getDimensionPixelSize(0));
            this.setGravity(Gravity.CENTER);
            if (isInEditMode()) {
                // cannot use a drawable in edit mode, so setting the background color instead
                // of a background resource.
                this.setBackgroundColor(getResources().getColor(0));
